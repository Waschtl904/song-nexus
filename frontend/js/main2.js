...
const App = {
    ...
    renderTracks() {
        ...
        if (featured.length === 0) {
            tracksList.innerHTML = '<div class="card" style="grid-column: 1/-1; text-align: center;"><p style="color: var(--color-text-secondary);">🎵 No tracks available</p></div>';
            return;
        }
        ...
        tracksList.innerHTML = featured.map(track => `
            <div class="card track-card">
                ...
                ${track.is_premium ? '<div class="track-badge">💰 Paid</div>' : '<div class="track-badge" style="background: rgba(0, 204, 119, 0.1); color: var(--color-accent-primary); border-color: var(--color-accent-primary);">🆓 Free</div>'}
                
                <button class="button" style="width: 100%; margin-top: 12px;" onclick="App.playTrack('${this.escapeHtml(track.audio_filename)}', ${track.is_premium}, '${this.escapeHtml(track.name)}')">
    ...
    renderBlogPosts(error = false) {
        ...
        blogList.innerHTML = latest.map(post => `
            <div class="card blog-card" onclick="window.location.href='blog/${post.slug}/'">
